...
y = tf.placeholder(tf.int32,[None,classes_num],name='y_input')
keep_prob = tf.placeholder(tf.float32,name='keep_prob')

def weight_variable(shape):
	return tf.Variable(tf.truncated_normal(shape,stddev=0.1),name='weight_variable')

def bias_variable(shape):
	return tf.Variable(tf.constant(0.1,shape=shape),name='bias_variable')
...
y = tf.one_hot(indices=tf.argmax(input=y,axis=1),depth=classes_num,dtype="int32",name='one-y')
...
writer = tf.summary.FileWriter('./my_graph2/4')
...
print("\n"+"="*50+"Test result"+"="*50)
#
print(sess.run(predictions["classes"],feed_dict={_X:batch[0],keep_prob:1.0}))
