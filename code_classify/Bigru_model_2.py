...
fw_gru_cell = tf.contrib.rnn.GRUCell(num_units=hidden_size,activation=None)
bw_gru_cell = tf.contrib.rnn.GRUCell(num_units=hidden_size,activation=None)
...
outputs, states = tf.nn.bidirectional_dynamic_rnn(fw_gru_cell, bw_gru_cell,inputs=X,
                                   initial_state_fw=init_state,initial_state_bw=init_state, dtype=tf.float32, time_major=False)
