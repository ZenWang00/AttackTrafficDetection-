...
#rnn_cell_fw = tf.contrib.rnn.BasicRNNCell(num_units=hidden_size,activation=None)
#rnn_cell_bw = tf.contrib.rnn.BasicRNNCell(num_units=hidden_size,activation=None)

#rnn_layers = [tf.nn.rnn_cell.RNNCell(size) for size in [256, 256]]

#multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)

#init_state = multi_rnn_cell.zero_state(batch_size, dtype=tf.float32)

#outputs, state = tf.nn.bidirectional_dynamic_rnn(cell=multi_rnn_cell, inputs=X,
                                   #initial_state=init_state, dtype=tf.float32, time_major=False)
state = stacked_bidirectional_rnn(hidden_size,layer_num,_X,timestep_size,batch_size)
print(state)
h_state = state[-1][1]
