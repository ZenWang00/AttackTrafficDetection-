...
# RNN network
rnn_input = tf.reshape(cnn_fc1_drop, [-1, rnn_timestep_size, rnn_input_size])

rnn_cell = tf.contrib.rnn.BasicRNNCell(num_units=rnn_hidden_layers,activation=None)

rnn_layers = [tf.nn.rnn_cell.BasicRNNCell(size) for size in [256, 256]]

multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)

init_state = multi_rnn_cell.zero_state(batch_size, dtype=tf.float32)

outputs, state = tf.nn.dynamic_rnn(cell=multi_rnn_cell, inputs=rnn_input,
                                   initial_state=init_state, dtype=tf.float32, time_major=False)

h_state = state[-1]

W_rnn = tf.Variable(tf.truncated_normal(shape=[256, classes_num], stddev=0.1), dtype=tf.float32)
bias_rnn = tf.Variable(tf.constant(0.15, dtype=tf.float32, shape=[classes_num]))
logits = tf.matmul(h_state, W_rnn) + bias_rnn
...
print("\ntest cost time :%f" % (time.time() - test_start))
print("\n" + "=" * 50 + "Test result" + "=" * 50)
