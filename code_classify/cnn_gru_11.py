...
gru_input_size = 160
gru_timestep_size = 10
gru_hidden_layers = 2
train_iter = 20000

# cnn network

_X = tf.placeholder(tf.float32, [None, img_shape],name="CNN_input_x")
y = tf.placeholder(tf.int32, [None, classes_num],name="CNN_input_y")
keep_prob = tf.placeholder(tf.float32,name="CNN_keep_prob")
...
# GRU network
gru_input = tf.reshape(cnn_fc1_drop, [-1, gru_timestep_size, gru_input_size],name="GRU_input_x")

gru_cell = tf.contrib.rnn.GRUCell(num_units=gru_hidden_layers,activation=None)

rnn_layers = [tf.nn.rnn_cell.GRUCell(size) for size in [256, 256]]

multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)

init_state = multi_rnn_cell.zero_state(batch_size, dtype=tf.float32)

outputs, state = tf.nn.dynamic_rnn(cell=multi_rnn_cell, inputs=gru_input,
                                   initial_state=init_state, dtype=tf.float32, time_major=False)
h_state = state[-1]

W_gru = tf.Variable(tf.truncated_normal(shape=[256, classes_num], stddev=0.1), dtype=tf.float32)
bias_gru = tf.Variable(tf.constant(0.15, dtype=tf.float32, shape=[classes_num]))
logits = tf.matmul(h_state, W_gru) + bias_gru
...
writer = tf.summary.FileWriter('./my_graph2/cn11')
